...
empty_packages_wo_rackcdn = []

for ep in empty_packages:
    if "rackcdn.com" not in ep[3].lower():
        empty_packages_wo_rackcdn.append(ep)

with open("Data\empty_packages_wo_rackcdn.csv", "w", encoding='utf-8', newline='') as f:
    ...
    csvfile.writerows(empty_packages_wo_rackcdn)
...
for ep in empty_packages_wo_rackcdn:
    ...
    if ".csv" in ep[3].lower():
        packages_csv.add(ep[0])

packages_other = packages_all - (packages_all & (packages_excel | packages_zip | packages_json |
                    packages_rackcdn | packages_cdn_forbidden | packages_cdx_small | packages_csv))

packages_csv_xls = packages_excel & packages_csv
packages_csv_xls = packages_csv_xls - (packages_csv_xls & packages_cdn_forbidden)
...
with open("Data\empty_packages_stat.csv", "w", encoding='utf-8', newline='') as f:
    ...
    csvfile.writerow(["Всего наборов с пустыми файлами csv: {}".format(len(packages_csv))])
    csvfile.writerow(["    (перечень - в empty_packages_csv.csv)"])
    csvfile.writerow(["Всего наборов, содержащих пустые файлы csv и xls одновременно: {}".format(len(packages_csv_xls))])
    csvfile.writerow(["    (перечень - в empty_packages_csv_xls.csv)"])
    ...
    csvfile.writerow(["Всего наборов с пустыми файлами по длинной ссылке: {} (rackcdn.com исключен)".format(len(packages_rackcdn))])
    ...
    csvfile.writerow(["Всего наборов с пустыми или мелкими файлами .cdx: {}".format(len(packages_cdx_small))])
    csvfile.writerow(["    (перечень - в empty_packages_cdx_small.csv)"])
    ...
    csvfile.writerow(["    (перечень - в empty_packages_cdn_forbidden.csv)"])
    csvfile.writerow(["Всего наборов с пустыми файлами, не вошедших в другие категории: {}".format(len(packages_other))])
    csvfile.writerow(["    (перечень - в empty_packages_other.csv)"])
...
with open("Data\empty_packages_csv.csv", "w", encoding='utf-8', newline='') as f:
    csvfile = csv.writer(f, 'customcsv')
    for p in packages_csv:
        csvfile.writerow([p])

with open("Data\empty_packages_csv_xls.csv", "w", encoding='utf-8', newline='') as f:
    csvfile = csv.writer(f, 'customcsv')
    for p in packages_csv_xls:
        csvfile.writerow([p])
