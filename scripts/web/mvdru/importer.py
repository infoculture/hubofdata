...
DATASETS_FILENAME = 'data/websites.csv'
...
class DataImporter:
    ...
    def __init__(self):
        ...
        self.started = True
        self.start_key = ''
        self.tags = [u'фас россии', 'RSS', u'новости', u'официально', u'госсайты']
        self.feedtype = 'RSS'
    ...
    def register(self, package):
        ...
        :param package:
        """
        id = hashlib.sha1(package['url']).hexdigest().encode('utf-8')[0:8]
        key = 'feed_' + id.lower()
        if not self.started:
        ...
        try:
            ...
            status = 404
        tags = [package['name'].lower(),]
        tags.extend(self.tags)

        name = self.feedtype + u' лента от ' + package['name'].decode('utf8')+ u' (%s)' %(package['url'],)
        description = u'Новостная лента от ' + package['name'].decode('utf8') + u' (%s)' %(package['url'],)

        resources = [{'name' : u"Новостная лента",
                             "format": self.feedtype, 'url': package['url'],
